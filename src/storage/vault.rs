...
impl Vault {
    pub fn allocate(p: &PathBuf) -> Result<PathBuf, DynamicError> {
        ...
        let path = f.allocate_header(&p)?;
        Ok(path)
    }

    pub fn allocate_header(&mut self, p: &PathBuf) -> Result<PathBuf, DynamicError> {
        let root_file = PathBuf::from(p).join(VAULT_N);
